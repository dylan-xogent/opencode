...
export type PatchDeps = {
  ...
  files: (dir: string, name: "awareness" | "opencode" | "tui") => string[]
}
...
function patchDir(input: PatchInput) {
  ...
  return path.join(root, ".awareness")
}

function patchName(kind: Kind): "awareness" | "tui" {
  if (kind === "server") return "awareness"
  return "tui"
}
