...
export function FormatError(input: unknown) {
  if (MCP.Failed.isInstance(input))
    return `MCP server "${input.data.name}" failed. Note, awareness does not support MCP authentication yet.`
  if (Provider.ModelNotFoundError.isInstance(input)) {
    ...
    return [
      ...
      `Or check your config (awareness.json) provider/model names`,
    ].join("\n")
  }
