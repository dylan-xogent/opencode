...
const WORKFLOW_FILE = ".github/workflows/awareness.yml"
      ...
      async fn() {
        {
          ...
          function printNextSteps() {
            ...
            prompts.outro(
              [
                ...
                "",
                "   Learn more about the GitHub agent - https://awareness.dev/docs/github/#usage-examples",
              ].join("\n"),
            )
          }
          ...
          async function installGitHubApp() {
            ...
            async function getInstallation() {
              return await fetch(
                `https://api.awareness.dev/get_github_app_installation?owner=${app.owner}&repo=${app.repo}`,
              )
                .then((res) => res.json())
...
jobs:
  awareness:
    if: |
      ...
      contains(github.event.comment.body, ' /awareness') ||
      startsWith(github.event.comment.body, '/awareness')
    ...
    steps:
      ...
      - name: Run awareness
        uses: dylan-xogent/opencode/github@latest${envStr}
        with:
...
export const GithubRunCommand = cmd({
  ...
  async handler(args) {
    await bootstrap(process.cwd(), async () => {
      ...
      const shareBaseUrl = isMock ? "https://dev.awareness.dev" : "https://awareness.dev"
      ...
      function normalizeOidcBaseUrl(): string {
        ...
        if (!value) return "https://api.awareness.dev"
