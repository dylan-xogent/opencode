...
import { createOpencodeClient, type Message, type OpencodeClient, type ToolPart } from "@awareness/sdk/v2"
...
export const RunCommand = cmd({
  command: "run [message..]",
  describe: "run awareness with a message",
  builder: (yargs: Argv) => {
    return yargs
      .positional("message", {
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running awareness server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
  ...
  handler: async (args) => {
    ...
    if (args.attach) {
      const headers = (() => {
        ...
        const username = process.env.OPENCODE_SERVER_USERNAME ?? "awareness"
