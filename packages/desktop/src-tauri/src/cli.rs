...
const CLI_INSTALL_DIR: &str = ".awareness/bin";
const CLI_BINARY_NAME: &str = "awareness";
...
pub fn install_cli(app: tauri::AppHandle) -> Result<String, String> {
    ...
    let temp_script = std::env::temp_dir().join("awareness-install.sh");
...
pub fn spawn_command(
    ...
    let mut cmd = if cfg!(windows) {
        if is_wsl_enabled(app) {
            ...
            let mut script = vec![
                "set -e".to_string(),
                "BIN=\"$HOME/.awareness/bin/awareness\"".to_string(),
                "if [ ! -x \"$BIN\" ]; then".to_string(),
                format!(
                    "  curl -fsSL https://awareness.dev/install | bash -s -- --version {} --no-modify-path",
...
pub fn serve(
    ...
    let envs = [
        ("OPENCODE_SERVER_USERNAME", "awareness".to_string()),
    ...
    .expect("Failed to spawn awareness");
