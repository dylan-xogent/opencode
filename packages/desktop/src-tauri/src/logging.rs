...
pub fn init(log_dir: &Path) -> WorkerGuard {
    ...
    let filename = format!("awareness-desktop_{timestamp}.log");
    ...
    let filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        if cfg!(debug_assertions) {
            EnvFilter::new("awareness_lib=debug,awareness_desktop=debug,sidecar=debug")
        } else {
            EnvFilter::new("awareness_lib=info,awareness_desktop=info,sidecar=info")
        }
    });
